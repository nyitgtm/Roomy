...
const FacultyDashboard: React.FC = () => {
    ...
    let currFaculty: Faculty | null = null;

    if (typeof window !== 'undefined') {
        const facultyData = localStorage.getItem('faculty');
        try {
            currFaculty = facultyData ? (JSON.parse(facultyData) as Faculty) : null;
        } catch (error) {
    ...
    const makeBooking = async () => {
        try {
            ...
            const res = await fetch('/api/booking/addbooking', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    reserverId: currFaculty?.faculty_id,
                    reserverType: 'Faculty',
                    roomId: selectedRoom,
                    date: selectedDate,
    ...
    const getBookingsForStudent = async () => {
        try {
            const res = await fetch('/api/booking/getbooking', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ reserverId: currFaculty?.faculty_id, reserverType: 'Faculty' }),
            });
